      ...
      <div ref={root} className="w-[1280px] max-w-[88vw]">
        ...
        >
          {data.objectives.map((o) => (
            ...
            >
              <div className="flex items-center justify-between text-[14px] uppercase tracking-[0.22em] text-ink/55">
                <span>
                  <span className="text-terracotta-300">{o.pillarRoman}</span> · {o.pillar}
                </span>
                <span className="text-ink/45 normal-case tracking-normal text-[14px]">{o.owner}</span>
              </div>
              <h3 className="mt-5 display text-[28px] leading-tight text-ink">{o.objective}</h3>
                  ...
                  {o.krs.map((kr, i) => (
                    <div key={kr.text} className="flex items-baseline gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-1 text-[19px] tnum text-ink/55">{kr.target}</div>
                      </div>
                    </div>
                  ))}
                </dl>
